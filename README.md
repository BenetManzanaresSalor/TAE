...
3. By default, your metric is expected to return a `float` for single-method evaluation (i.e., overriding `_evaluate_anonymization`) or a `Dict[str, float]` for multi-method evaluation (i.e., overriding `evaluate`). Nevertheless, you can return different values than `float`, as it is done in the [RecallPerEntityType](tae/metrics/recallperentitytype.py) class.
